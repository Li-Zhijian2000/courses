...
int posiList[MISSWORDTOTAL];
int posiListNext[MISSWORDTOTAL];
int postListSize;

int stack[MISSWORDTOTAL];
int stackSize;

char bufferMiss[MISSCHARTOTAL];
int bufferMissSize;
...
inline O3 INLINE 
    void updateMissWord(int index, int position) {
...
inline 
    void setMissWordName(int index, char word[])
{
    missWord[index].word = word;
}

static inline 
    int missWordComp(const void *lhs, const void *rhs)
{
...
inline 
    void quickSort()
{
    qsort(missWord + 1, missWordSize, sizeof(missWord[1]), missWordComp);
}

...
class NcAction extends Action {
    ...
    public void run(LinkedList<String> args) {
        try {
            ...
            if (Course.checkCid(cid) &&  Course.checkTitle(title) &&  Course.checkCapacity(capacity) && Course.checkTeachers(teachers)) {
                
                Course course = new Course(cid, title, teachers, capacity, personList);

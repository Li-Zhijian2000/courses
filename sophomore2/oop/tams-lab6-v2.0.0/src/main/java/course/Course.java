...
public final class Course implements Comparable<Course>, interObject {
    ...
    public static class ClassTime {
        ...
        public boolean conflict(ClassTime classTime) {
            return this.n == classTime.n && this.m == classTime.m
                    && rangeOverlap(this.fromWeek, this.toWeek, classTime.fromWeek, classTime.toWeek);
        }
    ...
    public Course(final String cid, final String title, List<String> teachers, int capacity, ClassTime classTime, PersonList personList) {
        this.cid = cid;
        this.title = title;
    ...
    public String toString() {
        ...
        return String.format("CID:%s,Name:%s,Teachers:%s,Capacity:%d/%d,Time:%s", 
            this.cid, this.title, Arrays.toString(teachersName.toArray()).replace(" ", ""),
            this.students.size(), this.capacity, this.classTime);
    }
